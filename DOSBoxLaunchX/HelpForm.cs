...
#pragma warning disable IDE1006 // Naming Styles
	private void HelpForm_Load(object sender, EventArgs ea) {
		txtHelp.Text =
			...
			What this allows is for different DOS games/apps to easily be run with a different configuration; a common requirement since no one configuration tends to work well with all different DOS games/apps. These config changes will be merged into an existing global set of config changes, so you can have some global changes that are always made by default (eg. mounting a directory as the C drive, a certain number of CPU cycles, etc.) and then a shortcut whose changes will be merged into this and override them if necessary (eg. different number of CPU cycles for this shortcut, a custom mouse sensitivity for this game, running the game's executable as part of autoexec, etc.) Shortcut files can also be created that just launch an interactive DOS shell with a particular configuration.
